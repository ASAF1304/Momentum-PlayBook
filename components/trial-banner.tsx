...
export function TrialBanner() {
  const { subscription: sub, subscriptionLoading, loading } = useAuth();
  const router = useRouter();
  const [dismissed, setDismissed] = useState(false);

  if (dismissed || loading || subscriptionLoading || !sub) return null;
  ...
  // Paid trial: only show in last 3 days
  ...
  const ctaLabel = isGrace
    ? isUrgent ? 'הוסף כרטיס — חודש חינם, אח"כ 50 ₪/חודש' : 'הוסף כרטיס אשראי'
    : 'הוסף כרטיס אשראי';
